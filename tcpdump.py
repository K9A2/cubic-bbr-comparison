...
def extract(line):
    ...
    seq 1059268509:1059280093, ack 1, win 58, options [nop,nop,TS val 1833200120
    ecr 46901062], length 11584
    ...
    """
    # 把这一行记录分成数组, 然后通过直接取数的方式来避免使用正则表达式, 提高效率.
...
def get_packets(lines, stats, role, algorithm, rtt, loss, local_logger):
    ...
    while i < length:
        ...
        if is_new_test(sport, cskp, dskp, DEFAULT_SERVER_PORT):
            ...
            nth_run += 1
            local_logger.info(str(nth_run) + '. Found new test at line: ' +
                              str(i) + ', cskp=' + cskp + ', dskp=' + dskp + '.')
    ...
    for key in keys:
        ...
        if receiver_stats[key]['packet_loss'] != 0:
            global_stats[key]['r/l'] = \
                float(sender_stats[key]['packet_retransmitted']) / float(receiver_stats[key]['packet_loss'])
        else:
        ...
        local_logger.info('packet_sent:                         %-d' % global_stats[key]['packet_sent'])
        local_logger.info('packet_loss:                         %-d' % global_stats[key]['packet_loss'])
        local_logger.info('packet_retransmitted:                %-d' % global_stats[key]['packet_retransmitted'])
        local_logger.info('on_wire_sent_sender (GB):            %-3.3f' % global_stats[key]['on_wire_sent_sender'])
        local_logger.info(
            'on_wire_retransmitted_sender (GB):   %-3.3f' % global_stats[key]['on_wire_retransmitted_sender'])
        local_logger.info('on_wire_sent_receiver (GB):          %-3.3f' % global_stats[key]['on_wire_sent_receiver'])
        local_logger.info(
            'on_wire_retransmitted_receiver (GB): %-3.3f' % global_stats[key]['on_wire_retransmitted_receiver'])
        local_logger.info('on_wire_total (GB):                  %-3.3f' % float(
            global_stats[key]['on_wire_sent_sender'] + global_stats[key]['on_wire_retransmitted_sender'] +
            global_stats[key]['on_wire_sent_receiver'] + global_stats[key]['on_wire_retransmitted_receiver']))
