...
public class Worker implements Runnable {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!header.senderPort.equals(controlSocketPort) && !header.senderPort.equals(dataSocketPort) &&
                    ...
                    // 输出上一个测试的结果
                    outputEvaluationResult(dataPacket);
     ...
     * 输出上一次测试的结果
     *
     * @param dataPacket 数据包发送记录
     */
    private void outputEvaluationResult(HashMap<Integer, Integer> dataPacket) {
        if (!dataPacket.isEmpty()) {
            int retransmittedPacket = 0;
            for (int value : dataPacket.values()) {
                retransmittedPacket += (value >= Constant.DATA_PACKET_LOSS_THRESHOLD) ? 1 : 0;
            }
            logger.info(dataPacket.size() + " packet recorded, " + retransmittedPacket + " packet retransmitted, " +
                    "retransmission rate = " + Constant.PERCENTAGE_FORMAT.format(
                    (double) retransmittedPacket / dataPacket.size() * 100) + "%");
            // 为新测试清空数据集
            dataPacket.clear();
        }
    }
